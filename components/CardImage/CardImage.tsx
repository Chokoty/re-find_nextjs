...
export default function CardImage({ data }: Props) {
  ...
  return (
    <>
      {/* 화면사이즈 768 미만(모바일 환경) */}
      <Link href={`/artwork/${id}`} className="md:hidden">
        <div className="group relative inline-block w-full">
          ...
          <div className="absolute inset-0 z-[2] hidden size-full items-center justify-center rounded-[20px] bg-blackAlpha-600 text-white group-hover:flex">
      ...
      <div className="group relative hidden w-full md:inline-block">
        ...
        <div className="absolute inset-0 z-[2] hidden size-full flex-col rounded-[20px] border-base border-whiteAlpha-700 bg-blackAlpha-600 text-white group-hover:flex">
          ...
          <Link
            className="flex h-[50px] w-full flex-1 items-center justify-center py-2 md:px-4"
            href={`/artwork/${id}`}
          >
            <div
              // className={styles.textBox}
              className="flex h-[34px] w-full items-center justify-center rounded-[11px] bg-gradient-to-tl from-pink-500 to-pink-300 px-2.5 py-2 text-xs font-semibold text-white transition hover:from-pink-400 hover:to-pink-200"
