...
export function useArtistInfo(nickname: string) {
  return useQuery(queryOptions.artistInfo(nickname));
}
...
export function useArtistArtworks({
  ...
  const {
    ...
    queryOptions.artistArtworks({ nickname, sortType, board })
  );
